...
export default function LeaveManagementTab({text,language,userRole,currentUserId,companyId}){
  ...
  async function fetchAll(){
    setLoading(true)
    const[{data:myEmpRecord},{data:apps},{data:emps}]=await Promise.all([
      supabase.from('employees').select('id').eq('auth_user_id',currentUserId).maybeSingle(),
      supabase.from('leave_applications').select('*').eq('company_id',companyId).order('created_at',{ascending:false}),
    ...
    if(!isFullAdmin&&myEmpId){
      const{data:approverFor}=await supabase.from('leave_approvers').select('employee_id').or(`approver1_user_id.eq.${myEmpId},approver2_user_id.eq.${myEmpId}`)
      const empIds=(approverFor||[]).map(a=>a.employee_id)
      filtered=filtered.filter(a=>empIds.includes(a.employee_id))
    }
  ...
  return(
    <div className="p-6">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-lg font-semibold text-gray-800">{language==='zh'?'年假管理':'Leave Management'}</h2>
      </div>
      ...
      {loading?<div className="text-sm text-gray-400 py-8 text-center">{text.loading}</div>
        :filtered.length===0?<div className="text-sm text-gray-400 py-8 text-center border-2 border-dashed rounded-lg">{language==='zh'?'暫無記錄':'No records'}</div>
        :(
                ...
                <div className="flex items-start justify-between">
                  ...
                  {app.status==='pending'&&(
                    <div className="flex gap-2 flex-shrink-0 ml-4">
                      <button onClick={()=>handleApprove(app)} disabled={savingId===app.id} className="px-3 py-1.5 text-xs bg-green-600 text-white rounded hover:bg-green-700 disabled:opacity-50">{savingId===app.id?'...':(language==='zh'?'批准':'Approve')}</button>
                      <button onClick={()=>{setRemarkModal(app);setRemarkText('')}} className="px-3 py-1.5 text-xs bg-red-500 text-white rounded hover:bg-red-600">{language==='zh'?'拒絕':'Reject'}</button>
                    </div>
                  )}
                </div>
              </div>
      ...
      {remarkModal&&(
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl p-6 max-w-sm w-full mx-4">
            ...
            <textarea value={remarkText} onChange={e=>setRemarkText(e.target.value)} className={inputClass+' resize-none'} rows={3} placeholder={language==='zh'?'請輸入拒絕原因（可選）':'Enter reason (optional)'}/>
